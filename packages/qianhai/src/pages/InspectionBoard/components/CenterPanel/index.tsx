...
const CenterPanel: React.FC<Iprop> = (props: any) => {
  const [disCardList, setDisCardList] = useState<any>([
    { key: '1', name: '新增病害数(个)', num: 29 },
    { key: '2', name: '巡检公里数(公里)', num: 240 },
    { key: '3', name: '设备在线率(%)', num: 100 },
  ]);
  const [disCardTypeList, setDisCardTypeList] = useState<any>([
    { key: '1', name: '管养总里程', num: '389', unit: '公里', img: '../../../assets/licheng.svg' },
    { key: '2', name: '管养桥梁', num: '5', unit: '座', img: '../../../assets/qiaoliang.svg' },
    { key: '3', name: '管养隧道', num: '4', unit: '座', img: '../../../assets/suidao.svg' },
  ]);
  ...
  useEffect(() => {
    const newlist = disCardList.map((it: any, index: any) => {
      /* eslint-disable */
      if (index === 0) {
        return { ...it, num: props.todayInfo?.diseaseCount || 29 };
      } else if (index === 1) {
        return { ...it, num: props.todayInfo?.todayMile || 240 };
      } else if (index === 2) {
        return { ...it, num: props.todayInfo?.online || 100 };
      } else {
        return { ...it };
      }
    });
