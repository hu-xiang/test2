...
const MarkPic: React.FC<Iprops> = (props) => {
  ...
  const handleAddRect = () => {
    ...
    ret.push({
      ...
      innerHeight: window.innerHeight * 0.7,
  ...
  const handleDel = () => {
    ...
    // }
    if (rectangles?.length) setRectangles([]);
  };
  ...
  const submit = async () => {
    ...
    // props.onOk(res);
    let bbox: any = [];
    const vw = bgImage?.naturalWidth / 960;
    const vh = bgImage?.naturalHeight / (window.innerHeight * 0.7);
    if (rectangles?.length) {
      rectangles?.forEach((item: any) => {
        bbox = [
          { X: item?.x * vw, Y: item?.y * vh },
          { X: (item?.x + item?.width) * vw, Y: item?.y * vh },
          { X: (item?.x + item?.width) * vw, Y: (item?.y + item?.height) * vh },
          { X: item?.x * vw, Y: (item?.y + item?.height) * vh },
        ];
      });
    }
    props.onOk(bbox);
            ...
            <div className={styles.utilsLayer}>
              <div className={styles.graphicType}>
                <span
                  ...
                  style={rectangles?.length ? { cursor: 'not-allowed' } : {}}
                >
