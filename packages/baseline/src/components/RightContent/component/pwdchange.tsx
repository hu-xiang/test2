...
const CrtPart: React.FC<Iprops> = (props) => {
  ...
  return (
    <Modal
      ...
      // onOk={() => crtnewpart()}
      closable={!(props?.platform && props?.platform === 'resetPwd')}
      className="pwdreset"
      footer={
        <div
          className={`${styles.bottomButton} ${
            props?.platform && props?.platform === 'resetPwd' ? `${styles.centerButton}` : null
          }`}
        >
          {props?.platform && props?.platform === 'resetPwd' ? null : (
            <Button
              className={`bottomCancelButton`}
              onClick={() => {
                props.onCancel();
              }}
            >
