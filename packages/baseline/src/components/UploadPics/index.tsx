...
const UploadPicList: React.FC<Iprops> = (props) => {
  ...
  const [uploadSize] = useState<number>(1);
  const [disPlaySize] = useState<number>(1);
  ...
  const uploadFailFile = async (item: any) => {
    try {
      ...
      Object.keys(initialState?.otherParams).forEach((key: any) => {
        formDataU.append(key, initialState?.otherParams[key]);
      });
      // formDataU.append('libraryId', initialState?.fileLibraryId);
      const res = await props.uploadFun(formDataU);
      if (res) {
        if (res?.status === 0) {
  ...
  useEffect(() => {
    ...
    if (totalNum > 0) {
      ...
      if (
        ...
        initialState?.fileStatusList[currentPage].status === 'uploading'
      ) {
        setContinueFlag(false);
        return;
      }
