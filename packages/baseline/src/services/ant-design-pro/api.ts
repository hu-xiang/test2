...
const tkName = getTokenName();
/** 获取当前的用户 GET /api/currentUser */

export async function currentUser() {
  return request(`/admin/user/front/info`, {
    params: { token: localStorage.getItem(tkName) },
  });
}
...
/** 租户管理：租户id和名称  */
export async function getTenName() {
  return request('/admin/tenant/queryTenant', {
    method: 'GET',
  });
}

/** 租户管理：根据租户查设备  */
export async function getTenDevice(params: any) {
  return request('/admin/tenant/queryDeviceByTenant', {
    method: 'GET',
    params,
  });
}
...
/** 登录接口 POST /api/login/account */
export async function freeLogin(body: any) {
  return request<API.LoginResult>(`/auth/jwt/defaultToken`, {
    method: 'POST',
    data: body,
  });
}
