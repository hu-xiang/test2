  ...
  const setIntervalData = () => {
    ...
    const setUpdateInfo = () => {
      if (cdata.current?.length > 1 && pie && !flagRef.current) {
        ...
        if (document.querySelector(`.${props?.contentClassName}` || '.customContent')) {
          document.querySelector(`.${props?.contentClassName}` || '.customContent')!.innerHTML =
            textContent;
        }
        const textTitle = cdata.current[cref.current].type || '总计';
        if (document.querySelector(`.${props?.titleClassName}` || '.customTitle')) {
          /* eslint-disable */
          document.querySelector(`.${props?.titleClassName}` || '.customTitle')!.innerHTML =
            textTitle;
        }
      ...
      title: {
        ...
        customHtml: (container: any, view: any, datum: any) => {
          ...
          return renderStatistic(
            ...
            props?.titleClassName || 'customTitle',
          );
        },
      },
      content: {
        ...
        customHtml: (container: any, view: any, datum: any) => {
          ...
          return renderStatistic(
            ...
            props?.contentClassName || 'customContent',
          );
        },
      },
    },
    ...
    <div className={styles['ring-box']}>
      ...
      {props?.isShowAvg ? (
        <div className={styles['avg-class-div']}>
          <span>平均分</span>
          <span className={styles['avg-class-score']}>90</span>
        </div>
      ) : null}
    </div>
  );
