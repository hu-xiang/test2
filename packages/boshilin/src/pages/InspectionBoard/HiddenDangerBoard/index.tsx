...
const HiddenDangerBoard: React.FC<Iprops> = () => {
  const [acciData, setAcciData] = useState<any>([]);
  ...
  const getInfo = async (sceneVal: any, projectVal: any) => {
    ...
    if (status === 0) {
      setMapData({
        list: data?.trackPointList?.length && data?.trackPointList[0] ? data?.trackPointList : [],
  ...
  const getSCeneMapData = async (
    ...
    if (rec?.status === 0) {
      ...
      if (rec?.data?.length) {
        ...
        let newCenter: any[] = [];
        if (newType) {
          const startIndex = imgDataList?.findIndex(
            (item: any) =>
              item?.longitude === rec?.data[0]?.longitude &&
              item?.latitude === rec?.data[0]?.latitude,
          );
          setStart(startIndex || 0);
        }
  ...
  const clearTreeSelect = (val: any) => {
    setSelectTreeData([`${val?.facId}_${val?.sceneId}_${roadId}`]);
  };

  const getAcci = async (id: any) => {
    const res = await getAcciData({ facilitiesId: id });
    if (res?.status === 0) {
      setAcciData(res?.data || []);
    }
  };
