...
import AcciModal from './AcciModal';
...
const iconAcciMarker = new AMap.Icon({
  // 图标尺寸
  size: new AMap.Size(22, 26),
  // 图标的取图地址
  image: 'images/acci.svg',
  // 图标所用图片大小
...
const iconAcciBigMarker = new AMap.Icon({
  // 图标尺寸
  size: new AMap.Size(41, 48),
  // 图标的取图地址
  image: 'images/acciBig.svg',
  // 图标所用图片大小
  imageSize: new AMap.Size(41, 48),
});

const iconHasNormalMarker = new AMap.Icon({
  // 图标尺寸
  size: new AMap.Size(22, 26),
  // 图标的取图地址
  image: 'images/icon-has-danger.svg',
  // 图标所用图片大小
  imageSize: new AMap.Size(22, 26),
  // 图标取图偏移量
  // imageOffset: new AMap.Pixel(-14, -28)
...
const iconHasBigMarker = new AMap.Icon({
  // 图标尺寸
  size: new AMap.Size(41, 48),
  // 图标的取图地址
  image: 'images/icon-has-danger-big.svg',
  // 图标所用图片大小
  imageSize: new AMap.Size(41, 48),
});

const iconNoNormalMarker = new AMap.Icon({
  // 图标尺寸
  size: new AMap.Size(22, 26),
  // 图标的取图地址
  image: 'images/icon-no-danger.svg',
  // 图标所用图片大小
  imageSize: new AMap.Size(22, 26),
  // 图标取图偏移量
  // imageOffset: new AMap.Pixel(-14, -28)
...
const iconNoBigMarker = new AMap.Icon({
  // 图标尺寸
  size: new AMap.Size(41, 48),
  // 图标的取图地址
  image: 'images/icon-no-danger-big.svg',
  // 图标所用图片大小
  imageSize: new AMap.Size(41, 48),
});
...
const EvaMap: React.FC<Iprops> = (props: Iprops) => {
  const [currentAcciInfo, setCurrentAcciInfo] = useState<any>();
  const [isAcciVisible, setIsAcciVisible] = useState<boolean>(false);
  const [mapMarkerData, setMapMarkerData] = useState<any>();
  ...
  const child1Ref: any = useRef();
  const nodeTypeRef: any = useRef();
  const [modalStyle, setModalStyle] = useState({ x: 0, y: 0 });
  const { mapData, mapInfo, placeInfo, handleCenter, trailData, sceneTrailData, acciData } = props;
  ...
  useEffect(() => {
    ...
    if (Object?.keys(trailData?.list || [])?.length > 0) {
      Object?.keys(trailData?.list || []).forEach((it: any) => {
        if (trailData?.list[it]?.length > 0) {
          const pathArr: any[] = [];
          trailData?.list[it]?.forEach((itr: any) => {
  ...
  useEffect(() => {
    ...
    if (mapData?.list?.length) {
      center = [mapData?.list[0]?.longitude, mapData?.list[0]?.latitude];
      zoom = 11;
    }
    ...
    map.on('click', (evt: any) => {
      ...
      if (prevMarkerEva) {
        prevMarkerEva.marker.setIcon(
          prevMarkerEva?.isHas ? iconHasNormalMarker : iconNoNormalMarker,
        );
        prevMarkerEva = undefined;
      }

      if (child1Ref && child1Ref.current) {
        child1Ref.current?.resetMaskClosableFlag(false);
      }
      setIsAcciVisible(false);
      if (prevMarkerAcci) {
        prevMarkerAcci.marker.setIcon(iconAcciMarker);
        prevMarkerAcci = undefined;
      }
  ...
  const hideModal = () => {
    ...
    setIsModalVisible(false);
    if (prevMarkerEva) {
      prevMarkerEva.marker.setIcon(prevMarkerEva?.isHas ? iconHasNormalMarker : iconNoNormalMarker);
      prevMarkerEva.marker.setOffset(normalOffset);
      prevMarkerEva = undefined;
    }
  ...
  const renderClusterMarker = (context: any) => {
    ...
    size = Math.round(30 + (context.count / mapData?.list?.length) ** (1 / 5) * 25);
  ...
  useEffect(() => {
    setMapMarkerData(undefined);
    hideModal();
    renderMarkerEva = (context: any) => {
      context.marker.topWhenClick = true;
      context.marker.bubble = true;
      context.marker.setzIndex(20);
      context.marker.setIcon(iconNormalMarker);
      context.marker.setOffset(normalOffset);
      context.marker.on('click', (_event: any) => {
        setClickInfo({
    ...
    function addCluster() {
      const gridSize = setGridSizeNum(props.mapData?.list?.length);
      AMap.plugin('AMap.MarkerCluster', () => {
        // 异步加载插件
        // eslint-disable-next-line no-new
        mapClusterEva = new AMap.MarkerCluster(map, props.mapData?.list, {
          gridSize, // 设置网格像素大小
    ...
    if (mapData?.list && mapData?.list.length > 0) {
      ...
      map.setCenter(
        firstClustMakerRef.current?._position || [
          mapData?.list[0]?.longitude,
          mapData?.list[0]?.latitude,
        ],
      );
    } else {
      /* eslint-disable */
  ...
  useEffect(() => {
    const unlisten = history.listen((location: any) => {
      if (location?.pathname !== '/hiddenDangerBoard') {
        ...
        if (acciMarkers && acciMarkers?.length) {
          map.remove(acciMarkers);
          acciMarkers = [];
        }
        district = null;
        map && map.destroy();
  ...
  useEffect(() => {
    renderMarkerEva = (context: any) => {
      ...
      let isHas = !!(context.data[0]?.geoStatus === 2);
      iconMarkerEva = isHas ? iconHasNormalMarker : iconNoNormalMarker;
      if (prevMarkerEva) {
        if (
          prevMarkerEva?.id &&
          context.data?.length &&
          ...
          iconMarkerEva = isHas ? iconHasBigMarker : iconNoBigMarker;
          offset = bigOffset;
          prevMarkerEva = { marker: context.marker, id: context.data[0]?.id, isHas };
        }
      }
      context.marker.topWhenClick = true;
      context.marker.bubble = true;
      ...
      context.marker.on('click', (event: any) => {
        map?.setCenter(context.data[0]?.lnglat);
        if (
          prevMarkerEva &&
          prevMarkerEva?.id &&
          context.data?.length &&
          ...
          prevMarkerEva.marker.setIcon(isHas ? iconHasNormalMarker : iconNoNormalMarker);
        }
        setMarkerInfo({
          ...context.data[0],
        });
        ...
        prevMarkerEva = { marker: event.target, id: context.data[0]?.id, isHas };
        event.target.setIcon(isHas ? iconHasBigMarker : iconNoBigMarker);
      });
    };

    function addCluster() {
      const gridSize = setGridSizeNum(mapMarkerData?.trackList?.length);
      // allMarker = [];
      AMap.plugin('AMap.MarkerCluster', () => {
        // 异步加载插件
        // eslint-disable-next-line no-new
        mapClusterEva = new AMap.MarkerCluster(map, mapMarkerData?.trackList, {
          gridSize, // 设置网格像素大小
          renderClusterMarker, // 自定义聚合点样式
          renderMarker: renderMarkerEva, // 自定义非聚合点样式
          maxZoom: 20,
    ...
    if (mapMarkerData?.id) {
      ...
      props?.clearTreeSelect(clickInfo);
    }
    // 数据中需包含经纬度信息字段 lnglat
    if (mapMarkerData?.trackList) {
      if (!isFirstFlag) {
        addCluster();
        setIsFirstFlag(true);
      } else {
      ...
      map.setCenter(
        firstClustMakerRef.current?._position || [
          mapMarkerData?.trackList[0]?.longitude,
          mapMarkerData?.trackList[0]?.latitude,
        ],
      );
    }
  }, [mapMarkerData?.id]);
  ...
  const renderMarker = (item: any) => {
    ...
    marker.on('click', (event: any) => {
      ...
      if (prevMarkerAcci && prevMarkerAcci?.id && prevMarkerAcci?.id !== item?.id) {
        prevMarkerAcci.marker.setIcon(iconAcciMarker);
      }
      setCurrentAcciInfo(item);
      setModalStyle(event.pixel);
      setIsAcciVisible(true);

      prevMarkerAcci = { marker: event.target, id: item?.id };
      event.target.setIcon(iconAcciBigMarker);
    });

    acciMarkers.push(marker);
  };

  useEffect(() => {
    if (map) map?.remove(acciMarkers);
    if (acciData?.length) {
      acciMarkers = [];
      for (let i = 0; i < acciData?.length; i++) {
        renderMarker(acciData[i]);
      }
    }
  }, [acciData]);
  ...
  return (
    <div className={styles.evationContainerClass}>
      ...
      {isAcciVisible ? (
        <AcciModal
          isModalVisible={isAcciVisible}
          hideModal={hideModal}
          markerInfo={currentAcciInfo}
          onRef={child1Ref}
          style={modalStyle}
        />
      ) : null}
